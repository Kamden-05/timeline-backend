...
def get_timeline_by_user(db: Session, user_id: int) -> list[Timeline]:
    statement = select(Timeline).where(Timeline.user_id == user_id).order_by(Timeline.created_at.desc())
